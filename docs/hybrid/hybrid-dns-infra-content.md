...
1. The workload will send the DNS request to Azure Firewall, since Azure Firewall's IP address is configured as custom DNS server in the virtual network as described in [custom DNS server](/azure/virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances).

1. Azure Firewall will forward the DNS requrest to the DNS Private Resolver's inbound endpoint.

1. If DNS Private Resolver finds a match in the rulesets associated with its outbound endpoints, it will forward the DNS request to the target specified in the rule, which should be the on-premises DNS servers.
